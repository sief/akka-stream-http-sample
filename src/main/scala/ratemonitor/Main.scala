...
object Main extends App {
  ...
  implicit val system = ActorSystem("ratemonitor")
  ...
  RateDao.setupDB.map { _ =>
    ...
    val rateMonitorRef = RateMonitorActor.init()
    val bindingFuture = bindWebsocket(rateMonitorRef)
    ...
    bindingFuture
      ...
      .onComplete(_ => RateMonitorActor.shutdown()) // and shutdown when done
  }


  private def bindWebsocket(actorRef: ActorRef) = {
      ...
      case req@HttpRequest(GET, path@(Uri.Path(ServicePath)), _, _, _) =>
        req.header[UpgradeToWebsocket] match {
          ...
          case Some(upgrade) =>
            ...
            // the publisher is the Source of real-time events that get generated by messages sent to realTimeActorRef
            ...
            // send subscribe message and continue only after subscription is confirmed
            (actorRef ? RateMonitorActor.Subscribe(realTimeActorRef)).map { case _ =>

              // unsubscribe when client connection closed
              val sink = Sink.onComplete(_ => actorRef ! RateMonitorActor.Unsubscribe(realTimeActorRef))

              // concatenate the stream from the dao with the real-time event stream
